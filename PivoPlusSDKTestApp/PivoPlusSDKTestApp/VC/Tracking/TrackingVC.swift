...
class TrackingVC: UIViewController {
  ...
  private var supportedTrackingModes: [TrackingType] = []
  ...
  private func setupSegmentControllTrackingTypeSelectionView() {
    let allModes: [(TrackingType, String)] = [
      (.object, "Object"),
      (.face, "Face"),
      (.human, "Body"),
      (.horse, "Horse"),
    ...
    segmentTrackingTypeSelection.removeAllSegments()
    supportedTrackingModes = allModes.map { $0.0 }
    for (index, mode) in allModes.enumerated() {
      segmentTrackingTypeSelection.insertSegment(withTitle: mode.1, at: index, animated: false)
    }
    segmentTrackingTypeSelection.selectedSegmentIndex = 0
    trackingType = supportedTrackingModes[0]
  }
  ...
  @IBAction func didTrackingTypeOptionChanged(_ sender: UISegmentedControl) {
    ...
    guard selectedIndex >= 0, selectedIndex < supportedTrackingModes.count else { return }
    self.trackingType = supportedTrackingModes[selectedIndex]
